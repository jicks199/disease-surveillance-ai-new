...
const HospitalRegistrationForm: React.FC = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.oxygen_refill_time_estimation.trim())
      newErrors.oxygen_refill_time_estimation =
        "Oxygen refill time estimation is required";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_VERCEL}/api/v1/hospital/register`,
        {
          method: "POST",
        ...
        <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
          ...
          {(apiError || successMessage) && (
            <motion.div
              ...
              className={`m-6 p-4 rounded-lg flex items-center gap-2 ${
                apiError
            ...
            <motion.div variants={itemVariants} className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
                {[
                  ...
                  {
                    name: "total_oxygen_capacity",
                    label: "Oxygen Capacity (liters)",
                  },
